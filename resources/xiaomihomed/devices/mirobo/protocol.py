...
class EncryptionAdapter(Adapter):
    ...
    def _decode(self, obj, context):
        try:
            ...
            #logging.debug("Unable to decrypt, returning raw bytes." + str(ex))
            return obj

        try:
            jsoned = json.loads(decrypted.decode('utf-8'))
        except:
            logging.error("unable to parse json, was: %s", decrypted)
            jsoned = b'{}'
            raise
...
Message = Struct(
    ...
    "header" / RawCopy(Struct(
        ...
        "devtype" / Enum(Default(Int16ub, 0x02d1), default=Pass, **xiaomi_devices),
        "serial" / Default(Int16ub, 0xd71d),
